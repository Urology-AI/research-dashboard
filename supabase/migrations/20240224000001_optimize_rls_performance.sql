...
-- Fixes auth.jwt() re-evaluation issue by using subquery pattern
...
-- Optimized Patients RLS Policies - Using subquery for performance
CREATE POLICY "patients_select_policy" ON patients
    FOR SELECT USING (
        (select auth.jwt() ->> 'role') IN ('admin', 'clinician', 'researcher')
    );
...
CREATE POLICY "patients_update_policy" ON patients
    FOR UPDATE USING (
        (select auth.jwt() ->> 'role') IN ('admin', 'clinician')
    );
...
-- Optimized Users RLS Policies - Very restrictive
...
CREATE POLICY "users_update_policy" ON users
    FOR UPDATE USING (
        (select auth.jwt() ->> 'role') = 'admin' OR
        ((select auth.jwt() ->> 'user_id') = id::text AND (select auth.jwt() ->> 'role') != 'admin')
    );
